...
> 本文供未来编码阶段使用。当前仓库仍处于纯文档阶段；只有用户明确授权后才执行以下任务。
...
- [ ] 建立 3D 主视图、左 Blockly、右 Inspector、底部控制和日志抽屉。
- [ ] 连接 Challenge 加载、状态机控制和 Blockly 只读状态。
- [ ] 实现当前 Block 高亮和相关编译错误定位。
- [ ] 展示关节、末端、voxel、程序指标和结果分解。
- [ ] 实现面板折叠、键盘焦点、按钮禁用和 1280×720 降级布局。
- [ ] 校准唯一 Challenge 和示例程序，使 Completion Score ≥80。
